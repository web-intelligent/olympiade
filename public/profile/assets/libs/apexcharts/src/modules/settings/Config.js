...
/**
 * ApexCharts Config Class for extending profile options with pre-defined ApexCharts config.
 *
 * @module Config
 **/
export default class Config {
  ...
  init({ responsiveOverride }) {
    ...
    if (opts && typeof opts === 'object') {
      ...
      // If profile has specified a dark theme, make the tooltip dark too
    ...
    // default-config < global-apex-variable-config < profile-defined-config

    // get GLOBALLY defined options and merge with the default config
    let mergedWithDefaultConfig = Utils.extend(newDefaults, window.Apex)

    // get the merged config and extend with profile defined config
  ...
  extendYAxis(opts, w) {
    ...
    // profile can provide either an array or object in yaxis config
    if (opts.yaxis.constructor !== Array) {
      // convert the yaxis to array if profile supplied object
