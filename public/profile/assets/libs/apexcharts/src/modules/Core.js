...
export default class Core {
  ...
  plotChartType(ser, xyRatios) {
    ...
    gl.series.map((series, st) => {
      let comboCount = 0
      // if profile has specified a particular type for particular series
      if (typeof ser[st].type !== 'undefined') {
        if (ser[st].type === 'column' || ser[st].type === 'bar') {
          ...
          // profile has specified type, but it is not valid (other than line/area/column)
          console.warn(
  ...
  isMultipleY() {
    // profile has supplied an array in yaxis property. So, turn on multipleYAxis flag
    if (
  ...
  setupBrushHandler() {
    ...
    // if profile has not defined a custom function for selection - we handle the brush chart
    // otherwise we leave it to the profile to define the functionality for selection
