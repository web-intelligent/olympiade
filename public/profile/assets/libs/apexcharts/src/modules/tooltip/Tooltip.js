...
export default class Tooltip {
  ...
  drawTooltip(xyRatios) {
    ...
    if (w.config.markers.size === 0 || w.globals.markers.largestSize === 0) {
      // when profile don't want to show points all the time, but only on when hovering on series
   ...
   */
  onSeriesHover(opt, e) {
    // If a profile is moving their mouse quickly, don't bother updating the tooltip every single frame

    const targetDelay = 100
    const timeSinceLastUpdate = Date.now() - this.lastHoverTime
    if (timeSinceLastUpdate >= targetDelay) {
      // The tooltip was last updated over 100ms ago - redraw it even if the profile is still moving their
  ...
  seriesHover(opt, e) {
    ...
    // if profile has more than one charts in group, we need to sync
