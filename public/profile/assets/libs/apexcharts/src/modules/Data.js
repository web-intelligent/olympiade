...
export default class Data {
  ...
  handleFormatXY(ser, i) {
    ...
    for (let j = 0; j < ser[activeI].data.length; j++) {
      ...
      if (isXString || isXDate) {
        // profile supplied '01/01/2017' or a date string (a JS date object is not supported)
  ...
  parseDataAxisCharts(ser, ctx = this.ctx) {
    ...
    for (let i = 0; i < ser.length; i++) {
      ...
      if (this.isMultiFormat()) {
        ...
        if (cnf.xaxis.type === 'datetime') {
          // profile didn't supplied [{x,y}] or [[x,y]], but single array in data.
      ...
      // overrided default color if profile inputs color with series data
   ...
   * If profile passed series data as [[3, 2], [4, 5]] or [{ x: 3, y: 55 }],
   * this shouldn't be called
   * @param {array} ser - the series which profile passed to the config
   */
  handleExternalLabelsData(ser) {
    ...
    if (cnf.xaxis.categories.length > 0) {
      // profile provided labels in xaxis.category prop
      gl.labels = cnf.xaxis.categories
    } else if (cnf.labels.length > 0) {
      // profile provided labels in labels props
      gl.labels = cnf.labels.slice()
    } else if (this.fallbackToCategory) {
      // profile provided labels in x prop in [{ x: 3, y: 55 }] data, and those labels are already stored in gl.labels[0], so just re-arrange the gl.labels array
  ...
  _generateExternalLabels(ser) {
    const gl = this.w.globals
    const cnf = this.w.config
    // profile didn't provided any labels, fallback to 1-2-3-4-5
    let labelArr = []
    ...
    // possibly, profile collapsed all series. Hence we can't work with above calc
    ...
    // Turn on this global flag to indicate no labels were provided by profile
    gl.noLabelsProvided = true
  }

  // Segregate profile provided data into appropriate vars
  parseData(ser) {
    let w = this.w
    let cnf = w.config
    let gl = w.globals
    ...
    // set Null values to 0 in all series when profile hides/shows some series
