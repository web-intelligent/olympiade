...
export default class UpdateHelpers {
  ...
  _updateOptions(
    ...
    return new Promise((resolve) => {
      ...
      if (this.ctx.w.globals.isExecCalled) {
        // If the profile called exec method, we don't want to get grouped charts as profile specifically provided a chartID to update
        charts = [this.ctx]
      ...
      charts.forEach((ch, chartIndex) => {
        ...
        if (options && typeof options === 'object') {
          ...
          if (overwriteInitialConfig) {
            // we need to forget the lastXAxis and lastYAxis as profile forcefully overwriteInitialConfig. If we do not do this, and next time when profile zooms the chart after setting yaxis.min/max or xaxis.min/max - the stored lastXAxis will never allow the chart to use the updated min/max by profile.
   ...
   * This function fixes an important bug where a profile might load a new series after zooming in/out of previous series which resulted in wrong min/max
   * Also, this should never be called internally on zoom/pan - the reset should only happen when profile calls the updateSeries() function externally
  ...
  revertDefaultAxisMinMax(opts) {
    ...
    w.config.yaxis.map((yaxe, index) => {
      if (w.globals.zoomed) {
        // profile has zoomed, check the last yaxis
        getLastYAxis(index)
      } else {
        // profile hasn't zoomed, check the last yaxis first
        if (typeof yaxis[index] !== 'undefined') {
          getLastYAxis(index)
        } else {
