...
class Range {
  ...
  setYRange() {
    ...
    cnf.yaxis.forEach((yaxe, index) => {
      // override all min/max values by profile defined values (y axis)
    ...
    // for horizontal bar charts, we need to check xaxis min/max as profile may have specified there
  ...
  setXRange() {
    ...
    if (gl.isXNumeric || gl.noLabelsProvided || gl.dataFormatXNumeric) {
      ...
      // override all min/max values by profile defined values (x axis)
