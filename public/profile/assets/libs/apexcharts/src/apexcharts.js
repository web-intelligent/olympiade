...
export default class ApexCharts {
  constructor(el, opts) {
    ...
    // Pass the profile supplied options to the Base Class where these options will be extended with defaults. The returned object from Base Class will become the config object in the entire codebase.
  ...
  /**
   * The primary method profile will call to render the chart.
   */
  render() {
    // main method
  ...
  create(ser, opts) {
    ...
    // Handle the data inputted by profile and set some of the global variables (for eg, if data is datetime / numeric / category). Don't calculate the range / min / max at this time
    ...
    // if there is some data to show or profile collapsed all series, then proceed drawing legend
    if (
      !gl.noData ||
  ...
  updateOptions(
    ...
    if (options.series) {
      ...
      // profile updated the series via updateOptions() function.
      // Hence, we need to reset axis min/max to avoid zooming issues
      this.updateHelpers.revertDefaultAxisMinMax()
    }
    // profile has set x-axis min/max externally - hence we need to forcefully set the xaxis min/max
    if (options.xaxis) {
  ...
  /**
   * Get all charts in the same "group" (including the instance which is called upon) to sync them when profile zooms in/out or pan.
   */
  getSyncedCharts() {
  ...
  /**
   * Allows the profile to provide data attrs in the element and the chart will render automatically when this method is called by searching for the elements containing 'data-apexcharts' attribute
   */
  static initOnLoad() {
   ...
   * instance of the chart in case profile has assigned chartID to the targeted chart.
